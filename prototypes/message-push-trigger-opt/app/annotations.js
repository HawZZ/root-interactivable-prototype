...
const notes={
  rules:[
    ...
    ["分类查询","推送规则 > 筛选区","可以按云计时器通知或事件通知查看规则。云计时器通知固定使用云端触发，并在符合条件时同日合并；事件通知仍按各自规则发送。","filter-bar"],
  ...
  rule1:[
    ...
    ["消息分类","步骤 1 > 基础信息","云计时器通知会固定使用云端触发；同一台设备当天有多条这类提醒时会放在一起发送。事件通知不合并，按各自规则处理。","message-editor"],
  ...
  rule3:[
    ...
    ["同日合并","步骤 3 > 同日合并","只有云计时器通知时生效：同一台设备当天的多条提醒会放在一起发送。","same-day-merge"],
