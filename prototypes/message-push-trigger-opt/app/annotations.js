...
const notes={
  rules:[
    ...
    ["统一多语言","页面标题 > 统一多语言","所有规则共用一张多语言大表。English 默认选中，其他语言按产品需要勾选。","product-language-entry"],
    ["分类查询","推送规则 > 筛选区","可以按耗材通知或事件通知查看规则。耗材通知会在符合条件时同日合并；事件通知仍按各自规则发送。","filter-bar"],
    ["规则列表","推送规则 > 表格","列表展示分类、触发条件、语言数量和投递方式。每条规则的「多语言」只编辑该规则；顶部「统一多语言」才维护产品全部规则。","rules-table"],
    ["后续版本","推送规则 > 后续能力","事件管理、Webhook、状态切换、优先级和自定义深链不在当前页面提供入口。","future-scope"]
  ],
  rule1:[
    ["编辑顺序","规则抽屉 > 顶部步骤条","先写要发送的内容，再设置何时触发，最后选择投递目标和发送方式。","step-nav"],
    ["消息分类","步骤 1 > 基础信息","耗材通知只可使用云端触发；同一台设备当天有多条这类提醒时会放在一起发送。事件通知不合并，按各自规则处理。","message-editor"],
  ...
  rule3:[
    ["投递目标","步骤 3 > 投递目标","默认写入 App 消息中心；也可发送到手机系统通知栏。","delivery-target"],
    ...
    ["同日合并","步骤 3 > 同日合并","只有耗材通知且使用云端触发时生效：同一台设备当天的多条提醒会放在一起发送。","same-day-merge"],
    ["保存检查","步骤 3 > 保存检查","保存前检查内容、触发条件、提醒时间和发送方式是否完整。","validation"]
  ],
  language:[
    ["选择语言","统一多语言 > 选择语言","English 始终选中。勾选其他语言后，这些语言才会出现在大表中并需要填写。","language-selection"],
    ["多语言大表","统一多语言 > 大表","每行是一个输入项，每列是一种语言；同一产品的全部规则在这里统一维护。","product-language-matrix"],
    ["导出大表","统一多语言 > 导出","可导出当前已选语言的表格，列顺序与页面一致。","product-language-intro"],
    ["保存检查","统一多语言 > 保存检查","只检查已选语言。没有选中的语言会保留数据，但不要求补齐。","product-language-validation"]
  ],
  ruleLanguage:[
    ["当前规则","规则多语言 > 说明","此抽屉只编辑从列表点击的那一条规则，不会同时打开其他规则的内容。","rule-language-intro"],
    ["多语言内容","规则多语言 > 编辑区","Title 和 Body 分两行维护；显示的语言范围沿用当前产品已经选择的语言。","rule-language-matrix"],
    ["保存检查","规则多语言 > 保存检查","只检查当前产品已选语言。保存成功后更新这条规则的语言数量与 English 标题。","rule-language-validation"]
  ]
};
