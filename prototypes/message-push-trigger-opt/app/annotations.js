...
const notes={
  rules:[
    ...
    ["规则入口","页面标题 > 新建推送规则","消息内容从属于规则，不创建独立模板；三步抽屉按触发、投递、消息顺序收敛配置。","page-heading"],
  ...
  rule2:[
    ["优先级约束","步骤 2 > 优先级","P0/P1/P2 只影响投递调度，不限制提醒模式。","priority-field"],
    ["模式互斥","步骤 2 > 提醒模式与频控","each 无最小间隔；discard/merge/stateSwitch 必填 5 分钟至 7 天窗口。","delivery-mode"],
    ["状态切换","步骤 2 > 状态切换提醒","仅事件触发；初始化可选开/关，每个有效事件按 deviceId 原子翻转，窗口末只发送最终状态内容。","delivery-mode"],
    ["安全硬顶","步骤 2 > 发布校验","滚动保护默认 72 小时/100 次，硬顶 168 小时/500 次，按稳定 ruleId + deviceId 跨版本累计。","validation"],
    ["频控时间线","步骤 2 > 投递时间线","演示首条入队、窗口内 suppressed/最后事件/最终状态，以及同 occurrence 不重复执行。","frequency-timeline"],
    ["发布校验","步骤 2 > 发布校验","校验已选语言、变量、cozy://、来源能力、webhook 安全状态、周期标识和窗口参数。","validation"]
  ],
  rule3:[
    ["规则内消息","步骤 3 > 推送消息","默认编辑语言为 English。标题和正文支持占位符按钮，内容归属规则，不产生独立模板实体。","message-editor"],
    ["占位符选择器","步骤 3 > 消息编辑","预设 10 个变量；当前触发无法提供的变量禁用，未知变量在发布时阻断。","placeholder-panel"],
    ["跳转模式","步骤 3 > 跳转页面","只允许不跳转、当前产品预设页面、自定义 cozy:// 深链。自定义链接校验协议、格式、长度和空格。","jump-mode"],
    ["语言入口","步骤 3 > 多语言","18 语言目录的 KV 矩阵从语言选择入口打开；只校验已勾选语言，English 始终 fallback。","language-entry"]
  ],
  language:[
    ...
    ["跳转一致性","多语言抽屉 > 跳转页面","跳转页面是规则顶层单值，在步骤 3 配置；语言抽屉只展示当前模式和链接。","message-link"],
