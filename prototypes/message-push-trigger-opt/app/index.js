...
import { appState, products, rules, newRule } from "./state.js";
...
function ruleRow(item) {
  ...
  const message = isSummary ? `<div class="primary-cell">${esc(item.title)}</div><div class="secondary-cell">${item.conditions.length} 个汇总条件 · 每日 ${esc(item.sendTime)}</div>` : `<div class="primary-cell">${esc(item.title)}</div><div class="secondary-cell">${esc(item.body)}</div>`;
...
function rulesView() {
  ...
  return `<div class="help-alert" data-anchor="p0-scope"><strong>本期规则：</strong>普通推送按一次触发独立发送；汇总推送在设定时间预测同设备当天的定时事项，每天最多发送一次。</div>
...
function typeChooser() {
  $("#overlayRoot").insertAdjacentHTML("beforeend", `<div class="modal-host" id="ruleTypeDialog"><section class="modal type-dialog" role="dialog" aria-modal="true" aria-labelledby="typeDialogTitle"><header class="modal-header"><div><h3 id="typeDialogTitle">选择推送规则类型</h3><p>类型创建后不能直接切换，避免丢失配置。</p></div><button class="icon-btn" data-type-close>×</button></header><div class="modal-body type-options"><button class="type-option" data-new-type="NORMAL"><span class="type-option-icon">1</span><span><strong>普通推送</strong><small>一个触发条件生成一条独立消息，可选逐条发送或仅发送首条。</small></span><b>选择 ›</b></button><button class="type-option" data-new-type="DAILY_SUMMARY"><span class="type-option-icon">2</span><span><strong>汇总推送</strong><small>在每天设定时间预测同设备当天的汇总条件，命中后发送一条消息。</small></span><b>选择 ›</b></button></div><footer class="modal-footer"><button class="el-btn" data-type-close>取消</button></footer></section></div>`);
