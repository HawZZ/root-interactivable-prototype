...
| 属性 | 内容 |
| --- | --- |
| 状态 | 2026-09-21 本地修订版：自研待注册 7 天、Tuya 待注册及分享码 1 小时、Meari 待接受 7 天；邮件/短信/H5拆为三套模板；完整注册提供 Plan A 同页复用与 Plan B 同标签页跳转两套评审方案；待注册撤销、待接受移除及共享中取消能力均已确认 |
| 更新日期 | 2026-09-21 |
...
3. 作为设备拥有者，当邀请或共享关系不再需要时，我想撤销待注册邀请、移除待接受邀请或取消共享中关系，以阻止后续建联或继续访问设备。
...
- 过期、撤销、移除或取消成功后记录自动从共享管理删除并释放额度，不展示对应终态或筛选项。
- 服务端保留终态、通知历史与审计，用于阻止旧通知、旧链接及迟到事件恢复权限，并确保撤销或过期不能重置频控。
- 同设备、同账号已有待注册邀请时阻止重复创建，不续期；需要再次触达时从该记录的操作菜单选择“重新发送通知”。
- 待注册记录的补发不新增记录、不重复占额。共享管理操作菜单按状态提供操作：待注册显示“重新发送通知、撤销邀请”；待接受显示“移除邀请”；共享中显示“取消共享”。

## 8. 邀请撤销与共享取消

- 三种前端可见状态均支持拥有者主动终止，但操作名称与业务结果不同：自研/Tuya `pending_registration` 使用“撤销邀请”；Meari `pending_acceptance` 使用“移除邀请”；全部厂商 `active` 使用“取消共享”。三项能力均已确认。
- 每项操作均需二次确认，前端不得乐观删除。仅云端及对应 SDK/关系服务最终返回成功后，记录才从共享管理移除并释放 1 条额度。
- 结果只分成功或失败。明确失败响应、超时和未知错误码均按失败处理：保留原记录、状态、额度与权限，并允许用户重新操作；不展示“处理中”或“结果未知”状态。
- 待注册撤销成功后，旧分享邀请不能再建立共享关系。撤销不清除通知历史；向同一账号重新发起仍受 5 分钟冷却、生命周期发送次数和固定日上限约束。
- 待接受移除成功后，旧系统通知、邮件、SDK 事件或待处理查询不得再次恢复邀请或完成接受；该记录释放额度。Meari 未注册注册指引本身不创建邀请，因此没有可撤销记录。
- 共享中取消成功后，被分享账号立即失去该设备的共享权限，旧消息、缓存或迟到事件不得恢复访问；接收者侧后续展示和只读取消消息沿用二期规则。
- 撤销、移除或取消成功后可重新发起分享；仍处于待注册、待接受或共享中时继续阻断同设备同账号重复创建。
...
| 邀请撤销与共享取消 | 产品能力已确认，待生产联调 | 自研/Tuya 待注册撤销、Meari 待接受移除及全厂商共享中取消均由服务端权威执行；成功才移除和释放额度，失败/超时/未知错误码保留原状态；旧链路和迟到事件不能恢复 |
...
| P3-AC-02 | 状态、额度与管理操作 | 仅三种可见状态合计 20 条；待注册提供补发与撤销，待接受提供移除，共享中提供取消；补发不新增记录或占额；Meari 注册指引无记录、不占额 | 共享管理 |
...
| P3-AC-09 | 撤销、移除、取消与过期 | 三种终止操作仅成功/失败；成功才删除记录、释放额度并使旧链路失效，失败保留原状态；自研待注册 7 天、Tuya 待注册及分享码 1 小时、Meari 待接受 7 天；到期自动释放且不清除通知历史 | 撤销与过期 |
...
| 待注册撤销 | 自研/Tuya 待注册 | 确认撤销，分别模拟成功与失败/超时/未知错误码 | 成功后删除记录、释放额度且旧分享邀请不能再建立共享关系；失败保留记录可重试 | P3-AC-02/09 |
| 待接受移除 | Meari 待接受 | 确认移除，分别模拟成功与失败/超时/未知错误码 | 成功后删除记录、释放额度且旧通知/SDK 事件不可接受；失败保留记录可重试 | P3-AC-02/09 |
| 共享中取消 | 任一厂商共享中 | 确认取消，分别模拟成功与失败/超时/未知错误码 | 成功后删除记录、释放额度并撤销接收者权限；失败保留关系、额度与权限可重试 | P3-AC-02/08/09 |
