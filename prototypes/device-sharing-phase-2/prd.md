...
| 已注册 Push、消息中心、已绑定验证邮箱邮件并行；原文短信仅用于未注册手机号 | 自研/Tuya 在最终建联后发送系统 Push、消息中心、适用邮件，并按本轮确认增加适用短信；Meari 仍通知有效待接受邀请，接受后才建联 | 厂商分支及渠道调整 | 恢复原通知闭环；新增自研/Tuya 已注册手机号短信 | P2-AC-04、P2-AC-05、P2-AC-06 |
| 未注册邮箱邮件、下载二维码、期限与注册指引 | 二期不发送、不展示成功结果 | 移出 | 三期能力草案，不提前承诺邀请 | P2-AC-03 |
| 未注册手机号短信及 HTML 注册指引，多语言，设备名称可展示，禁婴幼儿信息 | 未注册账号仍不触发；自研/Tuya 已注册手机号在建联后按本轮要求新增短信触达，复用现有短信模板 | 混合调整 | 未注册链路归三期；已注册建联短信归二期 | P2-AC-03、P2-AC-06 |
...
flowchart LR
  ...
  subgraph S["通知 / 系统 / SDK"]
    O5 -- "自研 / Tuya" --> S1["最终建联"]
    O5 -- "Meari" --> S2["创建待接受邀请"]
    S1 --> S6{{"系统 Push / 消息中心 / 邮件 / 短信"}}
    S2 --> S3{{"系统通知 / 邮件 / SDK 推送"}}
    S3 --> S4{"APP 前台或恢复"}
...
矩形为 APP 页面，双边框为既有客服组件，菱形为判断或弹窗，六边形为外部通知/SDK 事件。节点名称与原型五组业务场景及页面步骤一致；客服辅助分支仅明确入口，不展开客服内部流程。
...
flowchart TD
  ...
  W --> Z
  I --> J["异步发送系统 Push、消息中心、适用邮件及短信；失败不回滚"]
  J --> K{"APP 达到设备最低版本？"}
  K -- 否 --> L["设备暂不展示，通知引导升级"]
  K -- 是 --> M["按区域和当前权限展示设备"]
  L --> Z["结束"]
...
自研/Tuya 必须先由各自关系维护方确认最终建联成功，再异步触发下列通知。通知触发不参与关系事务：任一渠道发送失败均不得回滚、删除或重复创建共享关系；各渠道独立执行，不因单个渠道失败阻断其他渠道。

| 渠道 | 发送条件 | 标题/正文 | 点击与结果 |
| --- | --- | --- | --- |
| 系统 Push | 接收者已注册且允许系统通知；每条新建共享关系触发一次 | 标题、正文均为“{分享者昵称} 向你共享「{设备名称}」” | 正常版本：唤起 APP 并定位消息中心对应消息；低版本：使用 6.5.3 升级通知并跳应用商店 |
| 消息中心-系统消息 | 共享关系最终成立后写入一条系统消息 | 标题、正文均为“{分享者昵称} 向你共享「{设备名称}」” | 正常版本：进入设备列表并定位对应设备卡片；低版本：使用升级通知并跳应用商店 |
| 邮件 | 接收账号已绑定且验证邮箱；不论本次输入邮箱或手机号均按绑定邮箱判断 | 标题“{分享者昵称} 向你共享「{设备名称}」”；正文“{分享者昵称} 已将设备“{设备名称}”共享给你。请打开 Momcozy App 查看和使用。” | 同一共享关系一次，不催促；模板邮件无 APP 内接受动作；发送失败不影响关系 |
| 短信 | 接收账号已绑定可接收事务短信的手机号；自研/Tuya 最终建联后触发 | 复用模板 Rev.56：“{分享者昵称} 已将设备「{设备名称}」共享给你。请点击查看注册及使用指引：{HTML 指引链接}” | 同一共享关系一次，不催促；链接沿用模板配置，不在本需求新增落地页或参数 |
...
Meari 创建有效待接受邀请后，APP 不在前台时发送系统通知和适用邮件；APP 在前台时从 SDK 分享事件开始。通知只提醒打开 APP，不代表接受、不建立关系、不计成功。进入 APP 后必须由 SDK 事件或恢复查询匹配当前登录账号的有效邀请，再按 6.6 弹出接受/拒绝。Meari 接受成功后不重复发送“邀请”通知；本轮新增的自研/Tuya 建联短信不自动扩展到 Meari 待接受阶段。
...
| 系统 Push | 标题“设备共享”；正文“{分享者昵称} 已将设备「{设备名称}」共享给你，请升级至最新版本后查看和使用”；点击应用商店 | 使用邀请提醒进入 APP 时先引导升级；升级后恢复有效邀请 |
| 消息中心 | 标题和正文同系统 Push；点击应用商店 | 仅展示可恢复的有效邀请状态，不允许绕过升级接受 |
| 邮件 | 仅已绑定并验证邮箱；标题“{分享者昵称} 向你共享「{设备名称}」”；正文“{分享者昵称} 已将设备「{设备名称}」共享给你，请将 Momcozy App 升级至最新版本后查看和使用。”；无跳转 | 使用对应低版本邀请提醒；无邮件内接受动作 |
| 短信 | 仍按 6.5.1 复用现有短信模板；参考模板没有低版本短信变体，不依赖短信承担升级指引 | 本轮不新增 Meari 待接受短信 |
...
| 外部依赖 | BBM 清单、文案与通知模板 | 合规与通知团队确认；邮件沿用已注册/低版本模板，系统通知沿用原 PRD。模板 Rev.56 唯一短信仍为注册指引语义，须由模板负责人确认或修订已注册用户版本，不由本 PRD 自创文案 |
...
| 实施风险 | 通知页面或场景导航遗漏，评审误判渠道被删除 | 自研/Tuya 场景必须可定位 Push、消息中心、邮件、短信和版本页；二期短信仅用于已注册建联通知，不恢复未注册链路 |
...
| P2-AC-04 | 自研/Tuya 正常或低版本建联、Meari 待接受；模拟 Push 单渠道故障 | 自研/Tuya 最终建联后正常 Push 定位消息中心，低版本升级 Push 去应用商店；Meari 系统通知进入 APP 后经 SDK 事件/有效邀请恢复触发决策；失败不回滚关系或待接受状态，不阻断其他渠道 | [App Push](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=app-push) |
...
| P2-AC-10 | 用接收者最近版本与 IoT 设备最低 APP 版本构造支持/不支持样本；升级前取消/退出对照；区域错误样本 | 自研/Tuya 低版本仍建联并占额但设备隐藏，拥有者提示对方升级，Push/消息中心去应用商店、邮件无跳转，升级登录只同步仍有效关系；Meari 保持待接受，升级后恢复仍有效邀请再决策；跨区不展示数据，不新增已注册用户区域错误邮件触发规则 | [版本与区域](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=version-compat) |
...
| 自研/Tuya 建联通知 P2-AC-04、05、06 | 目标 APP 已注册且最终建联；分别有验证邮箱、绑定手机号 | 自研/Tuya；正常版本 | 查看 Push、消息中心、邮件、短信并点击可操作项 | 四类适用渠道均触发；Push 到消息中心、消息到设备；邮件/短信逐字使用模板；每关系每渠道一次 |
| 通知故障 P2-AC-04、P2-AC-05、P2-AC-06 | 关系已建立 | 分别让 Push、邮件或短信单渠道故障 | 查看关系与其他通道 | 关系保持；失败渠道不重复建联，其他适用渠道继续发送；未注册仍零通知 |
