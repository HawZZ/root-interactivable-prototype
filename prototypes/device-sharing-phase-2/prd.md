...
flowchart LR
  ...
  O6 --> E["无设备权限 / 空状态"]
  R5 --> E
  S2 -- "后台自然过期" --> S8["拥有者待接受记录自动删除并释放名额"]
  S8 --> S9["被邀请方不新增通知或状态提示"]
  P1["配网完成后邀请家人"] --> O3
```

矩形为 APP 页面，双边框为既有客服组件，菱形为判断或弹窗，六边形为系统通知或已注册邮箱对应邮件渠道；SDK 事件仅作为 APP 内部状态来源。短信/H5模板仅属于未注册手机号历史链路参考，不进入二期页面主流程。节点名称与原型五组业务场景及页面步骤一致；客服辅助分支仅明确入口，不展开客服内部流程。
