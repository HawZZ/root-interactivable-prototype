...
| 本人、格式、重复、上限、频次校验，提交 Loading，失败保留输入 | 删除等待注册重复提示分支；Meari 发送不占成功频次，接受建联时按实际成功日校验 | 规则调整 | 二期无等待注册状态，且 Meari 建联时机不同 | P2-AC-02、P2-AC-03、P2-AC-14 |
...
| 已注册 Push、消息中心、已绑定验证邮箱邮件并行；原文短信仅用于未注册手机号 | 三类设备均发送系统通知并同步消息中心；本次输入登录邮箱时叠加邮件，输入登录手机号时叠加短信。自研/Tuya 在最终建联后触发，Meari 在有效待接受邀请创建后触发 | 渠道触发规则调整 | 以本次指定账号类型唯一选择站外渠道；SDK 事件仅是 APP 内状态来源，不是触达渠道 | P2-AC-04、P2-AC-05、P2-AC-06 |
| 未注册邮箱邮件、下载二维码、期限与注册指引 | 二期不发送、不展示成功结果 | 移出 | 三期能力草案，不提前承诺邀请 | P2-AC-03 |
| 未注册手机号短信及 HTML 注册指引，多语言，设备名称可展示，禁婴幼儿信息 | 未注册账号仍不触发；本次指定的是已注册登录手机号时，三类设备均按各自通知时机复用现有短信模板 | 混合调整 | 未注册链路归三期；二期短信由本次输入账号类型决定 | P2-AC-03、P2-AC-06 |
...
共享发起成功率的“提交”发生在用户完成必要合规确认并实际发出请求时；进入页面、输入、按钮不可用、点击取消均不记分母。自研/Tuya 在关系最终建立后计成功；Meari 发送、拒绝、过期均不计成功，接受并最终建联后回补原请求月份。业务每日 2 次额度仍按实际建联日期计算，不能与指标归因月份混用。客诉指标由客服业务数据核验，不展开 VOC 台账，也不纳入 APP 埋点。
...
flowchart LR
  ...
  subgraph S["通知 / 系统 / SDK 状态"]
    O5 -- "自研 / Tuya" --> S1["最终建联"]
    O5 -- "Meari" --> S2["创建待接受邀请"]
    S1 --> S6{{"系统通知 + 消息中心记录 + 邮箱/手机对应渠道"}}
    S2 --> S3{{"系统通知 + 消息中心记录 + 邮箱/手机对应渠道"}}
    S3 --> S4{"APP 前台或恢复"}
...
矩形为 APP 页面，双边框为既有客服组件，菱形为判断或弹窗，六边形为系统通知或账号对应的邮件/短信渠道；SDK 事件仅作为 APP 内部状态来源。节点名称与原型五组业务场景及页面步骤一致；客服辅助分支仅明确入口，不展开客服内部流程。
...
flowchart TD
  ...
  P --> Q["系统通知及邮箱/手机对应渠道触达；APP 内事件/查询恢复有效邀请"]
  ...
  W --> Z
  I --> J["异步发送系统通知、消息中心及输入账号对应邮件/短信；失败不回滚"]
  J --> K{"APP 达到设备最低版本？"}
  K -- 否 --> L["设备暂不展示，通知引导升级"]
  K -- 是 --> M["按区域和当前权限展示设备"]
  L --> Z["结束"]
...
通知渠道由本次分享时拥有者输入的目标登录账号决定，不读取该账号的其他绑定方式扩展发送：输入邮箱时发送“系统通知 + 邮件”，输入手机号时发送“系统通知 + 短信”。系统通知面向已下载安装 APP 且注册登录的目标用户，并在消息中心同步保存系统消息。SDK 不作为用户触达渠道；Meari 的 APP 内 SDK 分享事件或待处理查询仅负责在 APP 中恢复邀请状态。通知触发不参与关系事务，任一渠道失败均不得回滚、删除或重复创建共享关系/邀请。
...
| 短信 | 本次指定账号是已注册登录手机号时发送；输入邮箱时不发送 | 复用模板 Rev.56：“{分享者昵称} 已将设备「{设备名称}」共享给你。请点击查看注册及使用指引：{HTML 指引链接}” | 同一关系/邀请一次，不催促；链接沿用模板配置，不在本需求新增落地页或参数 |

短信模板说明：参考文档当前唯一短信正文及 H5 仍是“注册及使用指引”语义，而二期接收者必须已注册。按本轮要求，原型逐字恢复该现有模板；上线前由通知模板负责人确认该模板可直接用于已注册用户，或在同一模板文档中修订为已注册用户版本。本 PRD 不擅自改写模板，也不因该内容依赖恢复未注册分享链路。

#### 6.5.2 Meari 待接受通知

Meari 创建有效待接受邀请后，按 6.5.1 发送系统通知，并按本次输入账号类型发送邮件或短信。APP 在前台时可直接从内部 SDK 分享事件开始展示流程，但 SDK 事件不是触达用户的通知。通知只提醒打开 APP，不代表接受、不建立关系、不计成功。进入 APP 后必须由内部 SDK 事件或待处理查询匹配当前登录账号的有效邀请，再按 6.6 弹出接受/拒绝。Meari 接受成功后不重复发送邀请通知。
...
| 低版本业务状态 | 共享关系保持“共享中”并占额，设备暂不展示；拥有者 Toast“共享成功，对方需要升级 App 到最新版本后查看” | 邀请保持“待接受”并占额，不提前建联；先升级，再恢复仍有效邀请并决策 |
...
| 短信 | 仅本次输入手机号时发送；参考模板没有低版本短信变体，不依赖短信承担升级指引 | 同左；仍须升级后恢复有效邀请 |
| 升级并登录 | 查询服务端最新关系，仅同步仍为共享中的设备；已取消/已退出不展示，无需拥有者重发 | 恢复未拒绝、未过期且仍有效的邀请；已拒绝/已过期不复活 |

原文评论中的升级弹窗不能自动覆盖原文正文的应用商店跳转规则。通知点击、APP 启动和设备页访问都必须以服务端最新关系、区域及版本结果为准，不能以旧 Push、邮件或短信恢复权限。

#### 6.5.4 终止与通知失败

| 情形 | 处理规则 |
| --- | --- |
| 拥有者取消 | 仅已建立关系支持二次确认后撤权、移除设备/列表成员并提示取消成功；接收者消息中心新增只读“设备共享已取消”通知；Meari 待接受无撤回 |
...
仅 Meari 适用。APP 不在前台时：系统通知 + 输入邮箱时邮件/输入手机号时短信 -> APP -> 内部 SDK 分享事件或待处理查询得到有效邀请 -> 强制接受/拒绝弹窗。APP 已在前台时从内部 SDK 事件开始；SDK 不作为用户触达渠道，通知链接本身不构成有效邀请或权限凭证。低版本按 6.5 先升级，再恢复有效邀请。
...
| 外部依赖 | BBM 清单、文案与通知模板 | 合规与通知团队确认；系统通知沿用原 PRD，邮件/短信按本次输入账号类型二选一。模板 Rev.56 唯一短信仍为注册指引语义，须由模板负责人确认或修订已注册用户版本，不由本 PRD 自创文案 |
...
| 实施风险 | 通知页面或场景导航遗漏，评审误判渠道被删除或并行发送 | 场景必须区分邮箱账号和手机号账号：两者都有系统通知/消息中心，邮箱只叠加邮件，手机号只叠加短信；SDK 事件不列为触达渠道 |
...
| P2-AC-03 | 区域/邮箱/手机号、本人/重复/未注册/格式、BBM 勾选及确认、每日 2 次、重试；三类关系维护方、自动映射前提及 SDK 运行失败；检查右上角客服入口 | US/CA +1、CN +86、其他邮箱；APP/后台仅以 APP 注册状态判断注册资格，不设独立 SDK 注册门槛；未注册无记录/邀请/通知/成功计数；自研/Tuya 直接建联，Meari 接受后核实建联；取消确认不提交请求；Meari 按实际接受成功日限同设备同账号 2 次；失败保留输入，幂等不重复计数；客服图标展示并调用既有客服组件，不新增客服流程或埋点 | [添加共享](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=add-share) |
...
| P2-AC-06 | 三类设备覆盖邮箱/手机号输入、正常/低版本、未注册及发送故障 | 输入登录邮箱：系统通知+邮件，不发短信；输入登录手机号：系统通知+短信，不发邮件。自研/Tuya 在建联后触发，Meari 在有效邀请创建后触发；同关系/邀请每渠道一次、不催促。未注册不发通知；单渠道失败不回滚业务状态。短信模板当前注册指引语义须在上线前确认，不在本 PRD 自改 | [邮件通知](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=email-notification)、[短信通知](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=sms-notification) |
...
| P2-AC-10 | 用接收者最近版本与 IoT 设备最低 APP 版本构造支持/不支持样本；升级前取消/退出对照；区域错误样本 | 自研/Tuya 低版本仍建联并占额但设备隐藏，拥有者提示对方升级，系统通知/消息中心去应用商店；邮箱输入对应的邮件无跳转，手机号输入对应的短信无低版本变体。Meari 保持待接受，升级后恢复仍有效邀请再决策；跨区不展示数据，不新增已注册用户区域错误邮件触发规则 | [版本与区域](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=version-compat) |
...
| P2-AC-14 | Meari 有效邀请，前台 APP 内 SDK 事件，或系统通知+账号对应邮件/短信进入 APP；接受/拒绝/失败/未知组合 | SDK 事件仅为 APP 内状态来源，不承担用户触达；强制二选一，无关闭/遮罩/Escape/返回；退出不等于拒绝；接受经云端+SDK 核验后先导航设备页再弹下一条，实际成功日计额度并关联原操作；拒绝成功停留底层页、释放待接受；失败保留弹窗重试，未知查询终态 | [邀请决策](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=meari-invitation)、[接受后的设备页](https://hawzz.github.io/root-interactivable-prototype/prototypes/device-sharing-phase-2/?page=recipient-device) |
...
| 账号对应通知 P2-AC-04、05、06 | 目标 APP 已注册；自研/Tuya 最终建联或 Meari 有效邀请创建 | 分别输入登录邮箱、登录手机号；正常版本 | 查看系统通知、消息中心及对应站外渠道 | 两类输入均有系统通知和消息中心；邮箱输入只叠加邮件，手机号输入只叠加短信；SDK 不触达用户；每关系/邀请每适用渠道一次 |
| 通知故障 P2-AC-04、P2-AC-05、P2-AC-06 | 关系已建立或有效待接受邀请已创建 | 分别让系统通知、对应邮件或对应短信单渠道故障 | 查看关系/邀请与其他适用渠道 | 业务状态保持；失败渠道不重复创建关系/邀请，其他适用渠道继续发送；未注册仍零通知 |
...
| 指标口径 P2-AC-16 | 固定窗口可追溯实际提交请求与最终关系 | 10 个去重提交请求：6 个直接建联、2 个 Meari 次月接受、2 个提交后失败；另有 2 次确认取消和重复重试 | 按原请求月份回补并核对实际接受日额度 | 分母 10、分子 8、80%；取消不计分母，重试去重，Meari 接受不新增请求，发送/拒绝/过期不计成功 |
...
| 强制弹窗 P2-AC-14 | Meari 已注册有效邀请 | 前台 APP 内 SDK 事件；后台系统通知+邮箱对应邮件/手机号对应短信 | 进入 APP；尝试关闭/遮罩/Escape/返回，再退出重进 | SDK 不作为触达渠道；有效事件/查询恢复后弹窗，不能关闭或绕过，退出不视为拒绝 |
...
| 后台 B01 | 按共享请求 ID 去重实际提交及最终关系事实。Meari 接受关联原请求并回补原请求月份，接受不新增分母；每日 2 次额度按实际建联日单独计算 | 共享发起成功率 |
| 后台 B02 | 生成月末设备快照：去重已绑定且支持共享设备、其中至少一条共享中关系的设备；以首个同口径完整月建基线并计算相对提升 | 设备共享率及基线 |

成功率按“最终成功建联的去重请求数 ÷ 实际提交的去重共享请求数”计算。提交后未注册、网络或 SDK 失败进入分母；确认取消不进入分母。自研/Tuya 低版本已建联仍计成功；Meari 待接受不计成功，接受结果关联原请求并回补原请求月份，接受不增加 A01；每日 2 次额度按实际接受成功日计算。发送、拒绝、过期不计成功。APP 注册自动映射 Tuya/Meari 两套 SDK 是用户确认但未经生产验证的前提，不新增 SDK 注册资格检查；映射异常或 SDK 运行失败按执行失败关联，不归为 APP 未注册。SDK 仅受理不能当作成功；同一业务请求只计一次。不按蓝牙/WiFi 拆分采集，不采集设备连接或通信结果。
